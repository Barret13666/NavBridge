// Copied verbatim from the BRouter project (https://github.com/abrensch/brouter,
// brouter-routing-app/src/main/aidl/btools/routingapp/IBRouterService.aidl),
// GPLv3. This is NOT BRouter's implementation -- just the interface
// definition, so this app can compile a client (BRouterClient.kt) that binds
// to the real BRouterService running inside the separately-installed
// BRouter app. Must stay byte-for-byte identical to BRouter's own copy:
// the AIDL compiler uses this to generate the Binder transaction stubs, and
// a mismatched interface would silently talk past the real service instead
// of failing loudly.
